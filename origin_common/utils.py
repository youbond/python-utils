...
TIMEDELTA_STRING_REGEX = re.compile(
    r"^\s*(?:nc)?\s*(\d+(?:\.\d+)?)\s*(m(?:onths?)?|y(?:ea)?(?:rs?)?|w(?:ee)?(?:ks?)?)?\*?\s*$",
    flags=re.IGNORECASE
)
...
def join_list(object_list: list, delimiter: str = ", ", last_delimiter: str = " & ") -> str:
    """
...
def timedelta_to_string(duration, only_initial=True, round_ndigits=None, only_quarter_years=True, for_quantlib=False):
    assert not for_quantlib or for_quantlib is only_initial, "QuantLib requires only initials"
    ...
    if for_quantlib and isinstance(value, float):
        ...
        return "{}{}".format(
            ...
            timedelta_to_string(string_to_timedelta("0.{}{}".format(decimal_part, units))),
        )
    if only_initial:
